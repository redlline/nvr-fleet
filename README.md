...
`MTX Toolkit` больше не входит в обязательный compose ядра. Теперь он подключается отдельно и собирается из исходников репозитория автора.

### Подготовка исходников add-on

```bash
bash scripts/setup_mtx_toolkit.sh
```

По умолчанию скрипт клонирует:

- repo: `https://github.com/dan246/mtx-toolkit.git`
- branch: `main`

При необходимости можно переопределить ветку или тег:

```bash
MTX_TOOLKIT_REPO_REF=v0.2.1 bash scripts/setup_mtx_toolkit.sh
```
...
После этого dashboard `MTX Toolkit` будет доступен отдельно от ядра проекта:

- UI: `http://YOUR_SERVER:3001`
- API: `http://YOUR_SERVER:5002`

### Что уже есть в System UI

Во вкладке `System` сейчас доступны:
...
В карточке площадки также есть:
...
- `docker-compose.mtx-toolkit.yml`
  Необязательный add-on для отдельного dashboard `MTX Toolkit`. Поднимается отдельно из исходников и не нужен для базового деплоя.
- `scripts/install.sh`
...
Если позже понадобится отдельный dashboard для MediaMTX, его можно поднять отдельно из исходников:
